...
const DAY=864e5,demo=[{id:'26',name:'2026 - 2027',startDate:'2026-06-01',endDate:'2027-05-31',status:'ACTIVE',autoActivate:true},{id:'27',name:'2027 - 2028',startDate:'2027-06-01',endDate:'2028-05-31',status:'UPCOMING',autoActivate:false},{id:'25',name:'2025 - 2026',startDate:'2025-06-01',endDate:'2026-05-31',status:'ARCHIVED',autoActivate:false}],states=['UPCOMING','ACTIVE','CLOSING','ARCHIVED'],blank={name:'',startDate:'',endDate:'',autoActivate:false};

const d=x=>{let z=new Date(`${x}T00:00:00`);
z.setHours(0,0,0,0);
return z},now=()=>{let z=new Date;
z.setHours(0,0,0,0);
return z},date=x=>d(x).toLocaleDateString('en-IN',{day:'2-digit',month:'long',year:'numeric'}),days=x=>Math.ceil((d(x)-now())/DAY),duration=(a,b)=>Math.round((d(b)-d(a))/DAY)+1,progress=(a,b)=>Math.max(0,Math.min(100,Math.round((now()-d(a))*100/(d(b)-d(a))))),autoStatus=(a,b)=>now()<d(a)?'UPCOMING':now()>d(b)?'ARCHIVED':'ACTIVE';

export default function AcademicYear(){const[years,setYears]=useState(demo),[search,setSearch]=useState(''),[filter,setFilter]=useState('ALL'),[modal,setModal]=useState(null),[selected,setSelected]=useState(null),[form,setForm]=useState(blank),[errors,setErrors]=useState({}),[notice,setNotice]=useState('This control center uses local demo data only.'),[activity,setActivity]=useState([{title:'Academic Year Created',detail:'2027 - 2028',time:'Today'},{title:'Academic Year Activated',detail:'2026 - 2027',time:'Yesterday'},{title:'Previous Year Archived',detail:'2025 - 2026',time:'Aug 18, 2026'}]);
...
setErrors(e);
return!Object.keys(e).length}function save(e){e.preventDefault();
if(!validate())return;
let item={...form,id:selected?.id||Date.now().toString(),name:form.name.trim(),status:selected?.status==='CLOSING'?'CLOSING':autoStatus(form.startDate,form.endDate)};
setYears(x=>selected?x.map(y=>y.id===selected.id?item:y):[...x,item]);
addActivity(selected?'Academic Year Updated':'Academic Year Created',item.name);
setNotice(`${item.name} has been ${selected?'updated':'added'} in this local demo.`);
close()}function move(status){let y=selected;
if(status==='ACTIVE')setYears(x=>x.map(a=>a.id===y.id?{...a,status}:a.status==='ACTIVE'?{...a,status:'ARCHIVED'}:a));
else setYears(x=>x.map(a=>a.id===y.id?{...a,status}:a));
addActivity(`Academic Year ${status==='ACTIVE'?'Activated':status==='CLOSING'?'Closing Started':'Archived'}`,y.name);
setNotice(`${y.name} moved to ${status.toLowerCase()} in the local lifecycle.`);
close()}function generate(){if(!active)return;
...
function Modal({type,selected,active,form,setForm,errors,close,save,generate,move}){let formMode=type==='add'||type==='edit',title=formMode?(type==='add'?'Add':'Edit')+' Academic Year':type==='generate'?'Generate Next Academic Year?':type==='activate'?'Activate Academic Year?':type==='close'?'Close Academic Year?':type==='archive'?'Archive Academic Year?':'Academic Year Details',confirm=type==='generate'?`${active?.name} → proposed next cycle`:selected?.name,desc=type==='activate'?'Activating this academic year changes the active academic context. The current active year will be archived.':type==='close'?'Closing prepares this academic year for archival.':type==='archive'?'Archived academic years cannot be reactivated through this control center.':'This will prepare the next annual cycle.';
return <div className="backdrop" onMouseDown={e=>e.target===e.currentTarget&&close()}><section className="modal" role="dialog" aria-modal="true" aria-labelledby="title"><button className="x" aria-label="Close dialog" onClick={close}>×</button><p className="eyebrow">Academic Year Control Center</p><h2 id="title">{title}</h2>{formMode?<form onSubmit={save} noValidate><label>Academic Year<input autoFocus value={form.name} onChange={e=>setForm(x=>({...x,name:e.target.value}))}/>{errors.name&&<em>{errors.name}</em>}</label><div className="formgrid"><label>Start Date<input type="date" value={form.startDate} onChange={e=>setForm(x=>({...x,startDate:e.target.value}))}/>{errors.startDate&&<em>{errors.startDate}</em>}</label><label>End Date<input type="date" value={form.endDate} onChange={e=>setForm(x=>({...x,endDate:e.target.value}))}/>{errors.endDate&&<em>{errors.endDate}</em>}</label></div>{errors.range&&<em>{errors.range}</em>}<label className="toggle"><input type="checkbox" checked={form.autoActivate} onChange={e=>setForm(x=>({...x,autoActivate:e.target.checked}))}/> Enable auto activation <small>Demo preference only;
 no background automation runs.</small></label><footer><button type="button" onClick={close}>Cancel</button><button className="solid">{type==='add'?'Add Academic Year':'Save Changes'}</button></footer></form>:<><p className="confirm">{confirm}</p>{type==='view'?<div className="details"><p>Academic year <b>{selected.name}</b></p><p>Date range <b>{date(selected.startDate)} — {date(selected.endDate)}</b></p><p>Status <b>{selected.status}</b></p></div>:<p className="muted">{desc}</p>}<footer><button onClick={close}>Cancel</button><button className={type==='archive'?'solid red':'solid'} onClick={()=>type==='view'?close():type==='generate'?generate():move(type==='activate'?'ACTIVE':type==='close'?'CLOSING':'ARCHIVED')}>{type==='view'?'Done':type==='generate'?'Generate Year':type==='activate'?'Activate':type==='close'?'Close Year':'Archive Year'}</button></footer></>}</section></div>}

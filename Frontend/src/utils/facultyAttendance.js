...
export const normalizeAttendanceRow = (row, { daily = false, date = '' } = {}) => {
  ...
  const statuses = { present: 'Present', absent: 'Absent', late: 'Late', halfday: 'Half Day', onleave: 'On Leave', lop: 'LOP', lossofpay: 'LOP', notmarked: 'Not Marked' }
  ...
  return { ...row, id: attendanceId || '', attendanceId: attendanceId || null,
    facultyId: String(row.facultyId ?? row.faculty?.facultyId ?? row.faculty?.id ?? row.employeeProfileId ?? (daily ? row.id : '') ?? ''),

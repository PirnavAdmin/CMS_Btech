...
export default function ElectiveManagement() {
    ...
    const loadCore = async () => { setLoading(true); setError(''); try { const [rawGroups, approvals, allocations, subjectRows] = await Promise.all([electiveManagementApi.getGroups(), electiveManagementApi.getApprovals(), electiveManagementApi.getAllocations(), facultyMasterApi.getSubjects()]); const groupsWithSubjects = await Promise.all(rawGroups.map(async group => { const id = group.electiveGroupId ?? group.groupId ?? group.id; let groupSubjects = []; try { groupSubjects = await electiveManagementApi.getGroupSubjects(id) } catch { /* A group without subjects remains visible for configuration. */ } return { ...group, id, groupId: id, course: String(group.courseId ?? ''), branch: String(group.branchId ?? ''), academicYear: group.academicYearName || '', semester: group.semesterName || '', status: Number(group.status) === 1 ? 'Open' : 'Inactive', subjects: groupSubjects.map(item => ({ ...item, id: item.subjectId, subjectName: item.subjectName, subjectCode: item.subjectCode })) } })); const allocationBySelection = new Map(allocations.map(item => [String(item.selectionId), item])); const combined = approvals.map(item => ({ ...item, id: item.selectionId, approvalStatus: item.approvalStatus, allocationStatus: allocationBySelection.get(String(item.selectionId))?.allocationStatus || 'Pending' })); setGroups(groupsWithSubjects); setReport(combined); setSubjects(unwrap(subjectRows).filter(subject => Number(subject.status ?? 1) === 1)) } catch (requestError) { setError(requestError.message || 'Unable to load elective groups.'); showError(requestError.message || 'Unable to load elective groups.') } finally { setLoading(false) } }
    ...
    const saveGroup = async event => { event.preventDefault(); const year = academicYears.find(item => String(item.academicYearName || item.name || '') === String(groupForm.academicYear)); const semester = semesters.find(item => String(item.semesterName || item.name || item.semesterNumber || '') === String(groupForm.semester).replace(/^Semester\s*/i, '')) || semesters.find(item => String(item.semesterName || item.name || '').toLowerCase() === String(groupForm.semester).toLowerCase()); const courseId = Number(groupForm.course); const branchId = Number(groupForm.branch); if (!year?.academicYearId && !year?.id || !semester?.semesterId && !semester?.id || !courseId || !branchId) return showError('Select valid course, branch, academic year, and semester IDs.'); setActionLoading(true); try { await electiveManagementApi.createGroup({ groupCode: groupForm.groupCode.trim(), groupName: groupForm.groupName.trim(), courseId, branchId, semesterId: Number(semester.semesterId || semester.id), academicYearId: Number(year.academicYearId || year.id), minSelections: Number(groupForm.minimumSelection), maxSelections: Number(groupForm.maximumSelection) }); showSuccess('Elective group created.'); setGroupModal(false); await loadCore() } catch (requestError) { showError(requestError.message || 'Unable to create elective group.') } finally { setActionLoading(false) } }

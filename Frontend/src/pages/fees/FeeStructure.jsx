...
function FacilityDetails({type,value,close,edit}){return <div className="fs-overlay"><section className="fs-editor details" data-export-record><header><div><span>{type==='hostel'?'HOSTEL':'TRANSPORTATION'} FEE STRUCTURE DETAILS</span><h2>{type==='hostel'?value.hostelName:value.route}</h2><p>{value.academicYearName} · {money(value.fee)}</p></div><ExportMenu mode="single" title={type === "hostel" ? value.hostelName : value.route} filename={`${type}_${value.routeCode || value.hostelName || value.id}_${value.academicYearName}`} recordSections={sectionsFromColumns("Fee Structure",value,type === "hostel" ? hostelFeeColumns : transportFeeColumns)} /><button onClick={close}><FiX/></button></header><div className="fs-editor-body"><FacilityPreview type={type} value={value}/></div><footer><button onClick={close}>Back</button><span/><button className="primary" onClick={edit}><FiEdit2 className="module-action-icon module-action-icon--edit" /> Edit</button></footer></section></div>}

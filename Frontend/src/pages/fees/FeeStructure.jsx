...
function Components({f,setF,errors,totals,amountPayable}){const update=(id,k,v)=>setF({...f,feeComponents:f.feeComponents.map(x=>x.id===id?{...x,[k]:v}:x)});return <><StepTitle title="Fee Components" text="One normalized source of truth for all academic charges."/><button className="primary" onClick={()=>setF({...f,feeComponents:[...f.feeComponents,feeComponent()]})}><FiPlus/> Add Fee Component</button>{errors.feeComponents&&<p className="error">{errors.feeComponents}</p>}<div className="fs-fee-components">{f.feeComponents.map((x,i)=><div key={x.id} className={errors[`component-${i}`]?'invalid-row':''}><input placeholder="Fee Head *" value={x.name} onChange={e=>update(x.id,'name',e.target.value)}/><select value={x.category} onChange={e=>update(x.id,'category',e.target.value)}><option value="">Select Category</option>{CATEGORIES.map(v=><option key={v}>{v}</option>)}</select><input type="number" min="0" placeholder="Amount *" value={x.amount} onChange={e=>update(x.id,'amount',e.target.value)}/><select value={x.frequency} onChange={e=>update(x.id,'frequency',e.target.value)}><option value="">Select Frequency</option>{FREQUENCIES.map(v=><option key={v}>{v}</option>)}</select><select value={x.requirement} onChange={e=>update(x.id,'requirement',e.target.value)}><option value="">Select Requirement</option><option>Mandatory</option><option>Optional</option></select><select value={x.refundable} onChange={e=>update(x.id,'refundable',e.target.value)}><option value="">Select Refundable</option><option>Non-refundable</option><option>Refundable</option></select><label><input type="checkbox" checked={x.concessionAllowed} onChange={e=>update(x.id,'concessionAllowed',e.target.checked)}/> Concession</label><label><input type="checkbox" checked={x.fineApplicable} onChange={e=>update(x.id,'fineApplicable',e.target.checked)}/> Fine</label><input placeholder="Due rule" value={x.dueRule} onChange={e=>update(x.id,'dueRule',e.target.value)}/><select value={x.status} onChange={e=>update(x.id,'status',e.target.value)}><option value="">Select Status</option><option>Active</option><option>Inactive</option></select><button onClick={()=>setF({...f,feeComponents:f.feeComponents.filter(c=>c.id!==x.id)})}><FiTrash2 className="module-action-icon module-action-icon--danger" /></button></div>)}</div><Totals totals={totals} amountPayable={amountPayable}/></>}

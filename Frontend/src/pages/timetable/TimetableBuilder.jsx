...
export default function TimetableBuilder({ step, setStep, contextFields, summary, scope, sources, entries, table, validScope, busy, setup, children, ...workspaceProps }) {
  ...
  return <section ref={root} className="tt-builder" aria-label="Timetable builder">
    <nav className="tt-stepper" aria-label="Timetable steps">{['Academic Setup', 'Period Setup', 'Generate & Manage'].map((title, index) => <button key={title} aria-current={visibleStep === index + 1 ? 'step' : undefined} className={visibleStep === index + 1 ? 'active' : ''} disabled={busy || (index === 1 && !validScope) || (index === 2 && (!table?.planning || setupDirty))} onClick={() => { setError(''); setStep(index + 1) }}><span>{index + 1}</span>{title}</button>)}</nav>
    {error && <p className="tt-error" role="alert">{error}</p>}
    {visibleStep === 1 && <AcademicSetupStep fields={contextFields} scope={scope} sources={sources} valid={validScope} busy={busy} existing={Boolean(table?.planning)} next={() => setStep(table?.planning ? 3 : 2)} />}
    {visibleStep === 2 && <PeriodSetupStep config={config} setConfig={setConfig} sources={sources} scope={scope} entries={entries} busy={busy} readOnly={table?.publicationStatus === 'published'} errors={setupErrors} back={() => setStep(1)} continueSetup={continueSetup} />}
    {visibleStep === 3 && table?.planning && <TimetableWorkspace {...workspaceProps} table={table} sources={sources} entries={entries} scope={scope} validScope={validScope} busy={busy} summary={summary} editSetup={() => setStep(2)}>{children}</TimetableWorkspace>}
  </section>
}

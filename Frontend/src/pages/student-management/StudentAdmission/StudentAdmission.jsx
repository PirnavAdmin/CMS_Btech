...
const CLEAN_START_KEY = 'pirnav-student-admissions-mock-cleared-v2'
...
const COURSE_FEES = { CSE: 340000, 'Artificial Intelligence & ML': 340000, 'Data Science': 340000, ECE: 300000, EEE: 280000, 'Mechanical Engineering': 250000, 'Civil Engineering': 240000 }
...
const empty = () => {
  ...
  fees: { tuitionFee: '', admissionFee: '4000', scholarshipAmount: '', hostelFee: '', transportFee: '', totalFee: '', paymentStatus: 'Pending', paymentPlan: '' },
...
>>>>>>> e2620c7501a11edef80c236f04cbc9051c2ae0df
const merge = row => {
  const base = empty()
  return {
    ...base, ...row,
...
const save = row => { const registrationNumber = row.application?.registrationNumber || row.application?.number; const next = { ...row, application: { ...row.application, number: registrationNumber, registrationNumber }, updatedAt: new Date().toISOString() }; localStorage.setItem(KEY, JSON.stringify([next, ...load().filter(item => item.id !== next.id)])); window.dispatchEvent(new Event('student-admissions-updated')); return next }
...
const validate = data => {
  ...
  if (!text(data.fees.paymentPlan)) errors['fees.paymentPlan'] = 'Select full payment or term-wise payment.'
...
function Field({ data, path, label, update, options, type = 'text', readOnly = false, error, placeholder, disabled = false, required = requiredPaths.has(path) }) {
  ...
  const numeric = /mobile|pincode|aadhaar/i.test(path)
  const maxLength = /aadhaar$/i.test(path) ? 12 : /pincode/i.test(path) ? 6 : /mobile/i.test(path) ? 10 : undefined
...
function ApplicantFeeStructure({ data, update, error }) {
  const tuition = Number(data.fees.tuitionFee || 0), admissionFee = Number(data.fees.admissionFee || 0), hostel = Number(data.fees.hostelFee || 0), transport = Number(data.fees.transportFee || 0)
  const firstYear = tuition + admissionFee + hostel + transport
  const fourYears = (tuition + hostel + transport) * 4 + admissionFee
  const firstTerm = Math.ceil(firstYear / 2), secondTerm = firstYear - firstTerm
  return <div className="sa-fee-structure-view"><section className="sa-applicant-fees"><header><FiInbox /><div><h2>Fee Structure</h2><p>Fees shown below are estimates based on the selected course and services.</p></div></header><dl><div><dt>Course / Branch</dt><dd>{display(data.academic.course)} · {display(data.academic.branch)}</dd></div><div><dt>Tuition Fee (per year)</dt><dd>{money(tuition)}</dd></div><div><dt>Admission Fee (one-time)</dt><dd>{money(admissionFee)}</dd></div>{data.admission.hostel === 'Yes' && <div><dt>Hostel Fee (per year) · {data.admission.hostelRoomType}</dt><dd>{money(hostel)}</dd></div>}{data.admission.transport === 'Yes' && <div><dt>Transportation Fee (per year)</dt><dd>{money(transport)}</dd></div>}</dl>{data.admission.scholarship === 'Yes' && <div className="sa-scholarship-note"><FiAlertCircle /><span><strong>Scholarship selected</strong>Final scholarship benefit will be adjusted after document verification and approval.</span></div>}<div className="sa-fee-estimates"><div><span>Estimated First-Year Total</span><strong>{money(firstYear)}</strong></div><div><span>Estimated Entire 4-Year Total</span><strong>{money(fourYears)}</strong></div><small>Tuition, hostel and transportation are annual charges. Admission fee is charged only once.</small></div></section><section className={`sa-payment-plan ${error ? 'invalid' : ''}`}><header><h3>Payment Preference <b>*</b></h3><p>Select how you prefer to pay the estimated fee.</p></header><div>{[['Full Payment','Pay the complete first-year amount'],['Term-wise Payment','Pay the first-year amount in two terms']].map(([value,label]) => <label className={data.fees.paymentPlan === value ? 'selected' : ''} key={value}><input type="radio" name="payment-plan" checked={data.fees.paymentPlan === value} onChange={() => update('fees.paymentPlan', value)} /><span><strong>{value}</strong><small>{label}</small></span></label>)}</div>{error && <small>{error}</small>}{data.fees.paymentPlan === 'Full Payment' && <div className="sa-term-breakdown"><header><strong>Full Payment Amount</strong><span>First academic year</span></header><dl><div><dt>Amount Payable</dt><dd>{money(firstYear)}</dd><small>Complete first-year estimated fee.</small></div></dl></div>}{data.fees.paymentPlan === 'Term-wise Payment' && <div className="sa-term-breakdown"><header><strong>Term-wise Estimate</strong><span>Two terms per academic year</span></header><dl><div><dt>First Term</dt><dd>{money(firstTerm)}</dd><small>Includes half of annual charges and the one-time admission fee share.</small></div><div><dt>Second Term</dt><dd>{money(secondTerm)}</dd><small>Remaining first-year estimated amount.</small></div></dl><p>From the second year onward, each term is approximately {money(Math.ceil((tuition + hostel + transport) / 2))} at the current fee structure.</p></div>}</section></div>
}
...
function DocumentReview({ data, edit }) { return <div className="sa-full-review sa-document-review"><ReviewSection title="Uploaded Documents" step={7} edit={edit} items={[...DOCUMENTS.map(([key,label,required]) => [label,data.documents?.[key]?.name,!required && data.admission.scholarship !== 'Yes']),['Other Certificates',(data.documents?.otherCertificates || []).map(item => item.name).join(', '),true]]} /></div> }
...
function AdmissionForm() {
  ...
  useEffect(() => {
    const tuitionFee = String(COURSE_FEES[data.academic.branch] || 0)
    const admissionFee = '4000'
    const hostelFee = String(data.admission.hostel === 'Yes' ? HOSTEL_FEES[data.admission.hostelRoomType] || 0 : 0)
    const transportFee = String(data.admission.transport === 'Yes' ? TRANSPORT_FEE : 0)
    const totalFee = String(Number(tuitionFee) + Number(admissionFee) + Number(hostelFee) + Number(transportFee))
    const hostelPreference = data.admission.hostel === 'Yes' ? (data.personal.gender === 'Female' ? 'Girls Hostel' : data.personal.gender === 'Male' ? 'Boys Hostel' : data.admission.hostelPreference) : ''
    if (data.fees.tuitionFee !== tuitionFee || data.fees.admissionFee !== admissionFee || data.fees.hostelFee !== hostelFee || data.fees.transportFee !== transportFee || data.fees.totalFee !== totalFee || data.admission.hostelPreference !== hostelPreference) queueMicrotask(() => setData(current => ({ ...current, admission: { ...current.admission, hostelPreference }, fees: { ...current.fees, tuitionFee, admissionFee, hostelFee, transportFee, totalFee } })))
  }, [data.academic.branch, data.personal.gender, data.admission.hostel, data.admission.hostelPreference, data.admission.hostelRoomType, data.admission.transport, data.fees.admissionFee, data.fees.hostelFee, data.fees.totalFee, data.fees.transportFee, data.fees.tuitionFee])
  ...
  useEffect(() => { const timer = window.setTimeout(() => { if (['DRAFT','CORRECTION_REQUIRED'].includes(data.status)) save(data) }, 700); return () => window.clearTimeout(timer) }, [data])
  ...
  const screens = [
    ...
    <><Section title="Application Information" icon={FiFileText} hint="System references remain readable and protected">{field('application.number','Registration Number',null,'text',true)}{field('application.date','Application Date',null,'date',true)}{field('admission.college','College',null,'text',true)}{field('admission.batch','Batch')}</Section><Section title="Student Services" icon={FiHome}>{field('admission.scholarship','Scholarship Required',['No','Yes'])}{data.admission.scholarship === 'Yes' && field('admission.scholarshipType','Scholarship Type')}{field('admission.hostel','Hostel Required',['No','Yes'])}{data.admission.hostel === 'Yes' && <>{field('admission.hostelPreference','Hostel Preference',['Boys Hostel','Girls Hostel'],null,true)}{field('admission.hostelRoomType','Room Type / Beds',Object.keys(HOSTEL_FEES))}</>}{field('admission.transport','Transportation Required',['No','Yes'])}{data.admission.transport === 'Yes' && field('admission.transportRoute','Transport Route')}</Section></>,
    <ApplicantFeeStructure key="fees" data={data} update={update} error={errors['fees.paymentPlan']} />,

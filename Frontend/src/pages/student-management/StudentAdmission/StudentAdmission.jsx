...
function AdmissionForm() {
  ...
  const semesterOptions=masters.semesters.map(item=>({id:item.semesterId??item.id,name:item.semesterName??item.name??`Semester ${item.semesterNumber??''}`,courseId:item.courseId??item.course?.courseId??item.course?.id,branchId:item.branchId??item.branch?.branchId??item.branch?.id})).filter(item=>item.id&&(!data.academic.courseId||!item.courseId||same(item.courseId,data.academic.courseId))&&(!data.academic.branchId||!item.branchId||same(item.branchId,data.academic.branchId)))
  ...
  const screens = [
    ...
    <Section key="academic" title="Academic Enrollment" icon={FiBookOpen} hint="Select academic year, course, branch, semester and admission categories.">{masterField('academic.academicYear','academic.academicYearId','Academic Year',yearOptions)}{field('academic.admissionType','Admission Type',ADMISSION_TYPES)}{field('academic.quota','Admission Quota',ADMISSION_QUOTAS)}{data.academic.quota === 'Other' && field('academic.quotaOther','Specify Admission Quota')}{masterField('academic.course','academic.courseId','Course',courseOptions,false,[['academic.branchId','academic.branch'],['academic.semesterId','academic.semester']])}{field('academic.courseCode','Course Code',null,'text',true)}{masterField('academic.branch','academic.branchId','Branch',branchOptions,!data.academic.courseId,[['academic.semesterId','academic.semester']])}{field('academic.branchCode','Branch Code',null,'text',true)}{masterField('academic.semester','academic.semesterId','Semester',semesterOptions,!data.academic.branchId)}{field('academic.studentCategory','Student Category',['General','SC','ST','BC','EWS','Other'])}{field('academic.regulation','Regulation')}{field('academic.entryType','Entry Type',['Regular','Lateral Entry','Transfer'])}</Section>,

...
const apiError = (error, fallback) => {
  ...
  if (status >= 500)
    return "Unable to save the department right now. Please check your entries and try again.";
...
const payloadFor = (value) => ({
  ...
  ...((value.collegeNumericId ?? value.collegeId) !== ""
    ? { collegeId: Number(value.collegeNumericId ?? value.collegeId) }
    : {}),
...
export default function DepartmentManagement() {
  ...
  const loadDepartments = async (searchTerm = query, page = currentPage) => {
    ...
    try {
      const response = searchTerm.trim()
        ? await searchDepartments(searchTerm.trim())
        : await getDepartments();
      const payload = response?.data?.data;
      const mapped = listFrom(response.data).map(mapDepartment);
      setAllDepartments(mapped);
      setItems(mapped);
      setServerTotalPages(Math.max(1, Math.ceil(mapped.length / itemsPerPage)));
    } catch (requestError) {
      setItems([]);
      setError(
        apiError(requestError, "Unable to load departments. Please try again."),
      );
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    loadDepartments(query, currentPage);
  }, [query, currentPage]);
  useEffect(() => {
    getColleges()
      .then((response) => {
  ...
  const loadDetail = async (item, nextScreen) => {
    ...
    setSelected(item);
    setForm(nextScreen === "form" ? { ...item, status: "" } : item);
    setError("");
  ...
  const confirmStatusChange = async () => {
    ...
    try {
      ...
      const updated =
        result?.id || result?.departmentId
          ? mapDepartment(result)
          : { ...item, status };
      await loadDepartments(query, currentPage);
  ...
  const save = async (event) => {
    ...
    if (!form.collegeId)
      return setError("Select a college for this department.");
    if (!form.status) return setError("Select a status for this department.");
    if (
      form.hodUserId !== "" &&
      (!Number.isInteger(Number(form.hodUserId)) || Number(form.hodUserId) < 0)
    )
      return setError("Head / In-Charge user ID must be a valid number.");
    if (
      items.some(
        (item) =>
          item.code === form.code.trim().toUpperCase() && item.id !== form.id,
      )
    )
    ...
    try {
      const response = form.id
        ? await updateDepartment(form.id, payloadFor(form))
        : await createDepartment(payloadFor(form));
      const result = recordFrom(response);
      ...
      setError(
        apiError(
          requestError,
          `Unable to ${form.id ? "update" : "create"} this department. Please try again.`,
        ),
      );
    } finally {
      setIsSaving(false);
  ...
  const saveHod = async (event) => {
    event.preventDefault();
    if (!String(form.hodName || "").trim())
      return setError("Enter the Head / In-Charge name.");
    setIsHodSaving(true);
    setError("");
    try {
      ...
      if (Number(form.hodUserId) > 0) {
        const response = await updateDepartment(
          form.id,
          payloadFor({ ...form, hodUserId: Number(form.hodUserId) }),
        );
        const result = recordFrom(response);
      ...
      setItems((current) =>
        current.map((item) => (item.id === form.id ? updated : item)),
      );
      setSelected(updated);
      closeToList();
    } catch (requestError) {
      setError(
        apiError(requestError, "Unable to assign the Head / In-Charge. Please try again."),
      );
    } finally {
  ...
  const field = (name, label, extra = {}) => (
    <label className={name === "description" ? "full-width" : ""}>
      <span>
        {label.endsWith(" *") ? (
          <>
            {label.slice(0, -2)} <b className="required-mark">*</b>
          </>
        ) : (
          label
        )}
      ...
      <div className="management-page department-management">
              ...
              {isLoading ? (
                ...
                <div className="department-no-results">
                  ...
                  <button
                    ...
                    onClick={() => loadDepartments(query, currentPage)}
                  >
                    Retry
                  </button>
                </div>
                ...
                <>
                          ...
                          <tr key={item.id}>
                            ...
                            <td className="department-actions-cell">
                              <div className="row-actions">
                                ...
                                >
                                  <FiUserPlus /> <span>Assign Head</span>
                                </button>
                                <button
                                  className={`status-action ${item.status === "Active" ? "danger" : "success"}`}
                                  title={
                                    item.status === "Active"
                                      ? `Mark ${item.name} inactive`
                                      : `Mark ${item.name} active`
                                  }
                                  aria-label={
                                    item.status === "Active"
                                      ? `Mark ${item.name} inactive`
                                ...
                                <button
                                  ...
                                  disabled={isDeleting}
                                >
                                  <FiTrash2 className="module-action-icon module-action-icon--danger" />
                                </button>
                              </div>
                            </td>
                          </tr>
                  ...
                  {!visible.length ? (
                    <p className="department-no-results">
            ...
            <section className="management-card department-form-card">
              ...
              <form onSubmit={save}>
                <div className="department-form-grid">
                  ...
                  {field("code", "Department Code *", {
                    placeholder: "e.g. CSE",
                    required: true,
                  })}
                  {field("description", "Description", { placeholder: "Describe this organizational department" })}
                  <label>
                    <span>
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        setForm({ ...form, collegeId: event.target.value })
                      }
                      required
                    >
          ...
          <Popup close={closeToList}>
            <section className="management-card hod-card">
              <Header
                ...
                subtitle={`Set the responsible head for ${form.name || "this department"}.`}
                back={closeToList}
                showClose={false}
              />
              <form onSubmit={saveHod}>
                {field("hodName", "Head / In-Charge Name *", {
                ...
                <Actions
                  cancel={closeToList}
                  submit={isHodSaving ? "Saving..." : "Assign Head"}
                  disabled={isHodSaving}
        ...
        {screen === "details" && selected && (
          <section className="department-profile-view">
            <div className="department-profile-topbar">
              <button className="secondary-button" onClick={closeToList}>
              ...
              {isDetailsLoading ? (
                ...
                <>
                  <div className="department-details__identity">
                    <div className="department-details__monogram">
                      {(selected.code || selected.name).slice(0, 3)}
                    </div>
                    <div>
                      <span>Organizational Department</span>
                      <strong>{selected.name}</strong>
                      <small>{selected.code || "No code"}</small>
                    </div>
                    <button
                  ...
                  <div className="detail-grid department-details__grid">
                    <div>
                      ...
                      <strong>{selected.code || "Not specified"}</strong>
                    </div>
                    <div>
                      <span>College</span>
                      <strong>
                    ...
                    <div>
                      ...
                      <strong
                        className={`department-detail-status ${selected.status.toLowerCase()}`}
                      >
                        {selected.status}
                      </strong>
                    </div>
                  </div>
                  {selected.description && <div className="detail-description"><span>Description</span><p>{selected.description}</p></div>}
                </>
              )}
              {error && (
                <p className="department-error" role="alert">
        ...
        {pendingStatus && (
          <StatusConfirmDialog
            ...
            description={pendingStatus.nextStatus === "Active"
              ? "The department will become available again for operations that use active departments."
              : "Associated record information is not available from the current APIs. Deactivating this department will only perform the existing department status-change operation."}
...
function Header({ title, subtitle, back, showClose = true }) {
  return (
    <div className="screen-title">
      ...
      {showClose && (
        <button
          className="department-popup-close"
          type="button"
          onClick={back}
...
function Actions({ cancel, submit, disabled, showCancelIcon = true }) {
  return (
    <div className="form-actions">
      <button
        ...
        disabled={disabled}
      >
        {showCancelIcon && <X aria-hidden="true" />} Cancel
      </button>
      <button type="submit" disabled={disabled}>
        <Save aria-hidden="true" /> {submit}
      </button>
    </div>
  );
}
  ...
  return (
    <div className="department-pagination">
      <div className="pagination-controls">
        ...
        <button
          className="pagination-btn"
          onClick={() => setPage((value) => Math.min(value + 1, total))}
          disabled={page === total}
        >
          Next
        </button>

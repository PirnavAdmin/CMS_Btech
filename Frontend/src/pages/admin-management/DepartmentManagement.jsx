...
import { FiCheckCircle, FiEye, FiLayers, FiEdit3, FiPlus, FiSave as Save, FiSearch, FiToggleLeft, FiToggleRight, FiUserPlus, FiX as X } from 'react-icons/fi'
...
import { createDepartment, getColleges, getDepartmentById, getDepartments, updateDepartment, updateDepartmentStatus } from '../../auth/collegeApi'
import './DepartmentManagement.css'

const empty = { id: null, name: '', code: '', collegeId: '', hodUserId: '', hodName: '', status: 'Active' }
...
const mapDepartment = (record) => {
  ...
  return { id: departmentId, name: record.departmentName ?? record.name ?? '', code: record.departmentCode ?? record.code ?? '', collegeId: record.collegeId ?? '', hodUserId, hodName, hod: hodName || (hodUserId !== '' ? 'Assigned HOD' : 'Not assigned'), status: active ? 'Active' : 'Inactive' }
}
const payloadFor = (value) => ({
  ...
  hodName: value.hodName.trim() || null,
})
...
export default function DepartmentManagement() {
  ...
  useEffect(() => {
    getColleges().then((response) => {
      ...
      setColleges(records.map((record) => ({ id: record.id ?? record.collegeId, name: record.name ?? record.collegeName ?? `College #${record.id ?? record.collegeId}` })))
    }).catch(() => setColleges([]))
  ...
  const saveHod = async (event) => {
    ...
    if (!form.hodName.trim()) return setError('Enter the HOD name.')
    setIsHodSaving(true); setError('')
    try { const response = await updateDepartment(form.id, payloadFor(form)); const names = savedHodNames(); names[form.id] = form.hodName.trim(); localStorage.setItem(HOD_NAMES_KEY, JSON.stringify(names)); const result = recordFrom(response); const updated = mapDepartment({ ...form, ...(result || {}), id: form.id, hodUserId: result?.hodUserId ?? form.hodUserId, hodName: result?.hodName || form.hodName }); setItems((current) => current.map((item) => item.id === form.id ? updated : item)); await loadDepartments(); closeToList() }
    catch (requestError) { setError(apiError(requestError, 'Unable to assign the HOD. Please try again.')) }
    finally { setIsHodSaving(false) }
  }
  const field = (name, label, extra = {}) => <label><span>{label.endsWith(' *') ? <>{label.slice(0, -2)} <b className="required-mark">*</b></> : label}</span><input value={form[name] ?? ''} onChange={(event) => setForm({ ...form, [name]: event.target.value })} {...extra} /></label>

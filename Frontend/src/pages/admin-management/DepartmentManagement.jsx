...
import { createDepartment, getColleges, getDepartmentById, getDepartments, getDepartmentsPaginated, searchDepartments, updateDepartment, updateDepartmentStatus } from '../../auth/collegeApi'
import './DepartmentManagement.css'

const empty = { id: null, name: '', code: '', collegeId: '', hodUserId: '', hodName: '', description: '', status: 'Active' }
...
const mapDepartment = (record) => {
  ...
  return { id: departmentId, name: record.departmentName ?? record.name ?? '', code: record.departmentCode ?? record.code ?? '', collegeId: collegeName || collegeId, collegeNumericId: collegeId, collegeName, hodUserId, hodName, description: record.description ?? '', hod: hodName || (hodUserId !== '' ? `User #${hodUserId}` : 'Not assigned'), status: active ? 'Active' : 'Inactive' }
}
...
export default function DepartmentManagement() {
  ...
  const loadDetail = async (item, nextScreen) => {
    ...
    try { const response = await getDepartmentById(item.id); const detail = mapDepartment(recordFrom(response)); setSelected(detail); setForm({ ...detail, collegeId: detail.collegeNumericId ?? detail.collegeId }); setItems((current) => current.map((entry) => entry.id === detail.id ? detail : entry)) }
  ...
  const saveHod = async (event) => {
    ...
    try { const response = await updateDepartment(form.id, payloadFor({ ...form, hodUserId: Number(form.hodUserId) })); const names = savedHodNames(); if (form.hodName.trim()) names[form.id] = form.hodName.trim(); else delete names[form.id]; localStorage.setItem(HOD_NAMES_KEY, JSON.stringify(names)); const result = recordFrom(response); const updated = mapDepartment({ ...form, ...(result || {}), id: form.id, hodUserId: result?.hodUserId ?? Number(form.hodUserId), hodName: result?.hodName || form.hodName }); setItems((current) => current.map((item) => item.id === form.id ? updated : item)); await loadDepartments(query, currentPage); closeToList() }

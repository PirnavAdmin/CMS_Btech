...
const TAB_FIELDS = {
  ...
  address: ['addressLine1', 'addressLine2', 'area', 'city', 'district', 'state', 'pincode', 'country'],
...
const initialValues = {
  ...
  addressLine1: '', addressLine2: '', area: '', city: '', district: '', state: '', pincode: '', country: 'India',
...
export default function AddCollege() {
  ...
  useEffect(() => {
    ...
    setPincodeStatus('Looking up postal locations...')
    fetch(`https://api.postalpincode.in/pincode/${values.pincode}`, { signal: controller.signal })
      ...
      .then(([result]) => {
        ...
        if (!offices.length) {
          setPostOffices([])
          setPincodeStatus('No postal location was found. Enter the address manually.')
          return
        }
        const first = offices[0]
        setPostOffices(offices)
        setValues((current) => ({ ...current, area: first.Name || '', city: first.Block || first.District || '', district: first.District || '', state: first.State || '', country: first.Country || 'India' }))
        setPincodeStatus(`${offices.length} postal location${offices.length === 1 ? '' : 's'} found.`)
      })
      .catch((error) => {
        if (error.name !== 'AbortError') {
          setPostOffices([])
          setPincodeStatus('Pincode lookup is unavailable. Enter the address manually.')
        }
      })
  ...
  const update = ({ target: { name, value } }) => {
    ...
    setValues((current) => name === 'pincode'
      ? { ...current, pincode: next, area: '', city: '', district: '', state: '' }
      : { ...current, [name]: next })
    if (name === 'pincode') {
      setPostOffices([])
      setPincodeStatus('')
  ...
  const submit = async () => {
    ...
    try {
      ...
      const college = {
        ...
        university: values.universityName.trim(), address: [values.addressLine1, values.addressLine2, values.area, values.city, values.district].filter(Boolean).join(', '),
  ...
  return <DashboardLayout><main className="add-college">
    ...
    <form onSubmit={(event) => { event.preventDefault(); submit() }} noValidate>
      ...
      {section('College Information', 'Core identity and affiliation details.', <>
        <Field label="College Name" name="collegeName" values={values} errors={errors} touched={touched} onChange={update} required maxLength={120} />
        <Field label="College Code" name="collegeCode" values={values} errors={errors} touched={touched} onChange={update} required maxLength={12} />
        ...
        <Field label="University Name" name="universityName" values={values} errors={errors} touched={touched} onChange={update} required maxLength={120} />
      ...
      {section('Address', 'Official postal address of the institution.', <>
        <Field label="Address Line 1" name="addressLine1" values={values} errors={errors} touched={touched} onChange={update} required maxLength={150} />
        <Field label="Address Line 2" name="addressLine2" values={values} errors={errors} touched={touched} onChange={update} maxLength={150} />
        ...
        <Field label="District" name="district" values={values} errors={errors} touched={touched} onChange={update} readOnly />
      ...
      {activeTab === 'contact' && section('Contact Information', 'Public institutional contact channels.', <>
        <Field label="Official Contact Number" name="contactNumber" values={values} errors={errors} touched={touched} onChange={update} required maxLength={10} inputMode="tel" />
        ...
        <Field label="Official Email" name="email" type="email" values={values} errors={errors} touched={touched} onChange={update} required maxLength={120} />
        <Field label="Website" name="website" type="url" values={values} errors={errors} touched={touched} onChange={update} maxLength={160} />
      </>)}
      ...
      {activeTab === 'accreditation' && section('Accreditation Details', 'Current accreditation standing and validity.', <>
        ...
        <Field label="Accreditation Body" name="accreditationBody" values={values} errors={errors} touched={touched} onChange={update} maxLength={80} />
        <Field label="Accreditation Grade" name="accreditationGrade" values={values} errors={errors} touched={touched} onChange={update} maxLength={20} />
    ...
    {dialog === 'preview' && <Dialog title="Preview College" actions={<><button className="ac-secondary" onClick={() => setDialog(null)}>Back to Edit</button><button className="ac-primary" onClick={submit} disabled={submitting}>{submitting ? 'Creating College...' : 'Submit'}</button></>}><div className="ac-preview">{values.logo && <img src={values.logo} alt="College logo" />}<h3>{values.collegeName}</h3><p><b>Code:</b> {values.collegeCode} · {values.collegeType}</p><hr/><h4>College Information</h4><p>{values.universityName}</p><h4>Address</h4><p>{[values.addressLine1, values.addressLine2, values.area, values.city, values.district, values.state, values.pincode, values.country].filter(Boolean).join(', ')}</p><h4>Contact</h4><p>{values.contactNumber} · {values.email}<br/>{values.website || 'No website provided'}</p><h4>Administration</h4><p>{values.principalName || 'Not provided'} {values.principalEmail && `· ${values.principalEmail}`}</p><h4>Accreditation</h4><p>{values.accreditationStatus}{values.accreditationBody && ` · ${values.accreditationBody}`}</p></div></Dialog>}

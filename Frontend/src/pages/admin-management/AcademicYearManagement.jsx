...
setSaving(true);
try{if(status==='ACTIVE')await academicYearApi.activate(selected.id);else await academicYearApi.deactivate(selected.id);addActivity(`Academic Year ${status==='ACTIVE'?'Activated':'Deactivated'}`,selected.name);setNotice(`${selected.name} has been ${status==='ACTIVE'?'activated':'deactivated'} successfully.`);close();await loadYears()}catch(error){setNotice(error.message||'Unable to update the academic year status.')}finally{setSaving(false)}}async function generate(){if(!active||saving)return;
...
const action=x=><div className="ay-actions"><button onClick={()=>openView(x)}>View</button><button onClick={()=>edit(x)}>Edit</button>{x.status==='UPCOMING'&&<button className="primary" onClick={()=>{setSelected(x);
setModal('activate')}}>Activate</button>}{x.status==='ACTIVE'&&<button className="primary" onClick={()=>{setSelected(x);
setModal('deactivate')}}>Deactivate</button>}</div>;

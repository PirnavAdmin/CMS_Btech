...
const action=x=><div className="ay-actions"><button title="View academic year" aria-label="View academic year" onClick={()=>openView(x)}><FiEye/></button><button title="Edit academic year" aria-label="Edit academic year" onClick={()=>edit(x)} disabled={x.status==='ARCHIVED'||isPresentYear(x)}><FiEdit2/></button>{isPresentYear(x)&&<button className="current-action" title="Present year" aria-label="Present year" disabled><FiToggleLeft/></button>}{x.status==='ARCHIVED'&&<button className="archived-action" title="Archived" aria-label="Archived" disabled><FiXCircle/></button>}{x.status==='UPCOMING'&&<button className="primary" title="Activate academic year" aria-label="Activate academic year" onClick={()=>{setSelected(x);

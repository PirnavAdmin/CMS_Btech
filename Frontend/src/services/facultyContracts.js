...
export const normalizeLeaveRequest = row => ({ ...row, id: String(row.leaveRequestId ?? row.requestId ?? row.id ?? ''), status: leaveStatus(row.statusName ?? row.requestStatus ?? row.status), facultyId: String(row.facultyId ?? row.employee?.facultyId ?? row.employee?.id ?? row.faculty?.facultyId ?? row.faculty?.id ?? ''), policyId: String(row.policyId ?? ''), typeId: String(row.leaveTypeId ?? row.typeId ?? ''), from: String(row.fromDate ?? row.from ?? '').slice(0, 10), to: String(row.toDate ?? row.to ?? '').slice(0, 10), applied: row.appliedOn ?? row.appliedDate ?? row.applied ?? row.createdAt, days: row.days == null && row.totalDays == null ? null : Number(row.days ?? row.totalDays) })

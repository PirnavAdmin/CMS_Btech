...
import { emptyDemo, normalizeDemo, validateDemo, submitDemoRequest } from '../api/demoRequest'
...
export default function RequestDemo({ onClose }) {
  ...
  return <dialog ref={dialog} className="rd-dialog" aria-labelledby="rd-title" onCancel={e => { e.preventDefault(); if (!lock.current) onClose() }}>
    ...
    <section className="rd-main">{result ? <div className="rd-success" role="status"><FiCheckCircle /><h1 id="rd-title" tabIndex={-1} data-success>Request submitted</h1><p>Thank you for contacting the college.</p><p>Your enquiry has been submitted successfully.</p>{result.reference && <><small>Reference ID</small><strong>{result.reference}</strong></>}<div className="rd-footer"><button className="lp-button" onClick={() => { onClose(); document.getElementById('home')?.scrollIntoView() }}>Return to Homepage</button><button className="rd-secondary" onClick={onClose}>Close</button></div></div> : <><p className="lp-eyebrow">LET’S EXPLORE YOUR CAMPUS NEEDS</p><h1 id="rd-title">Request information or a demo</h1><p className="rd-intro">Share your contact and institution details. Fields marked <b>*</b> are required.</p>
      ...
      <fieldset disabled={busy}><legend>Your enquiry</legend><div className="rd-grid">{field('institution', 'Organization / College (optional)', { autoComplete: 'organization' })}</div></fieldset>
      <label className="rd-field" htmlFor="rd-message"><span>Purpose / Message <b>*</b></span><textarea id="rd-message" name="message" rows={4} minLength={10} maxLength={1000} value={values.message} onChange={update} disabled={busy} required aria-invalid={Boolean(errors.message)} aria-describedby={errors.message ? 'rd-message-error' : 'rd-message-help'} /><small id="rd-message-help">10–1,000 characters. Tell us what you would like to know.</small>{errors.message && <small id="rd-message-error" className="rd-error">{errors.message}</small>}</label>

...
export default function ExportMenu({ rows = [], columns, filename, title, loading = false, scope = 'Current filtered results', unavailable = '', screen, exportParams, mode = 'list', recordSections, reportType = '', onDownload }) {
  ...
  return <div className="export-control" ref={root} data-no-print onKeyDown={event => { if (event.key === 'Escape') { setOpen(false); trigger.current?.focus() } }}>
    ...
    {open && !disabled && <div className="export-options" id={id}><small>{single ? 'Current record only' : `${scope} (${rows.length})`}</small><button type="button" disabled={!single && !rows.length && !supportsServerExport} onClick={() => run(single ? downloadRecord : supportsServerExport ? (onDownload || download) : exportToCsv, single ? 'Record downloaded successfully' : `${csvLabel} downloaded successfully`)}><FiDownload aria-hidden="true" /> {reportLabel ? `Download ${reportLabel}` : 'Download CSV'}</button><button type="button" disabled={!single && !rows.length} onClick={() => run(single ? printRecord : printResults, 'Print preview opened. Choose Print or Save as PDF in your browser.')}><FiPrinter aria-hidden="true" /> Print / Save as PDF</button></div>}
